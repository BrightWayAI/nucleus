...
- **Writers:** briefing `/brief` (creates), cortex `/listen` Step 1.5g and `/morning` Step 4.6 (append or replace `## Reflection`), optional cortex `/end-day` Step 4 (same reflection shape), briefing `/process-brief` (appends `### Processed annotations`)
- **Readers:** briefing next morning's `/brief` Yesterday's Reflection card; cortex `/listen` Step 1.5 maps stable item ids back to titles/nodes and runs the unmarked-item inference pass
- **Format:** see briefing `commands/brief.md` markdown twin template
- **Critical contract:** the `## Reflection` section format MUST match among cortex `/listen` Step 1.5g, `/morning` Step 4.6, optional `/end-day` Step 4, and briefing `/brief`. If any writer changes the section header or bullet shape, briefing breaks silently.
...
- **Folder-binding invariant (2026-09-18):** every schedule reads/writes `<config-root>`, so registration requires verified access from the intended execution environment. In Cowork, `create_trigger`/`update_trigger` requests `requires_local_device=true` and `folders=[<config-root>]`; `list_triggers` must return both a non-`NONE` `derived_state.folders_state` and the resolved root in `derived_state.folders`. Failed verification must not produce an active registration record. `/register-schedules --verify` reads scheduler state without mutating the task (it may update local `last_verified_at`).
- **OpenAI boundary:** those Cowork tool names and response fields are not portable APIs. A Codex local project setting or cached `ACTIVE` flag alone does not prove access. Require an exposed host scheduler's equivalent live environment/path verification plus a root read/write probe in the scheduled context; otherwise report binding verification unavailable and return manual definitions. Never fabricate Cowork fields or mark an unverifiable schedule healthy. See [OpenAI setup](OPENAI_SETUP.md#scheduled-listen-verification).

`<config-root>/plugins/ops/schedule-runs/<schedule>/<run-id>.json`
- **Writer:** scheduled workflow when the host permits local receipt writes. Cortex `/listen` Step 1.5j specifies `outcome`, `state_source` (`state.json | artifact-db | none`), and brief-mining counts; its Step 0 root gate specifies `status: "failed"`/`error_code: "config_root_unreachable"` here, at an authorized fallback, or on stdout if no writable location is reachable. The candidate has schema and lifecycle gaps noted below; it does not yet guarantee a terminal receipt on every exit.
...
- **Path audit (2026-09-18):** both pinned writers use `plugins/ops/schedule-runs/`, not `memory/staged/queues/receipts/` as stated in the original handoff. No path migration shipped; do not introduce a second receipt store based on that handoff.
- **Known candidate incompatibility:** Cortex 4.28.0 Step 1.5j emits `outcome: "succeeded"`, while ops 0.13.0 Step 5 defines `outcome: "success"` and `/status` accepts `status: "succeeded"` or `outcome: "success"`. The early failure path also adds singular `error_code` alongside a schema that defines `error_codes`. These must be reconciled in the owning plugins before promotion. Step 1.5j precedes the remaining ingest steps, so its receipt does not yet prove whole-run completion. The health contract requires a terminal receipt from the same host/task, not merely a brief-mining checkpoint or scheduler history.
...
- **Readers:** briefing `/brief` Step 0.A2 preflight (reads yesterday's doc back before the closures pass), cortex `/listen` Step 1.5a preflight (reads `<target_date>`'s doc back before mining). The pointer locates state; it does not grant access or establish that the current host exposes a database tool.
...
- **OpenAI boundary:** preserve the local v0.7.0 state-file contract. Read a hosted database only through an actually available, authorized capability; otherwise report it unavailable and retain existing local state. Do not invent `artifact-capabilities`, database calls, or a runtime pointer for a Markdown brief. The pointer lacks a provider/tool binding and freshness/conflict protocol, so automatic cross-host readback is not certified. Never let an unverified hosted snapshot replace newer explicit local actions.
...
- **Writers:** growth `/relationships-action` when `action: snoozed`; cortex `/listen` Step 1.5h mirrors known-person outreach snoozes from the brief ledger
...
- **Ownership:** this remains the single source growth reads. `<config-root>/briefs/.snooze-ledger.json` is a separate item-keyed briefing source; only cortex `/listen` synchronizes qualifying outreach entries between them.
- **Version:** added in growth v0.1.1
...
## Cross-plugin: briefing state ↔ cortex `/listen` and `/morning`
...
- **Writers:**
  ...
  - optional cortex `/end-day` Step 5 invokes briefing `/brief` for tomorrow using the same `todays-brief` id and format
- **Readers:**
  - briefing `/process-brief` Step 1 (reads `tasks` + `annotations` + `outreach_actions`)
  - cortex `/listen` Step 1.5 (mines explicit state and unmarked-item evidence into closures, snoozes, reflection, and staged memory proposals)
  - optional cortex `/end-day` Step 2c/4.0 (same-day synchronous path; writes `.state.processed` for `/listen` idempotency)
...
- **Canonical 6-section format (briefing v0.11.0 / state schema v0.7.0):**
  ...
  4. **Outreach Queue** — per-contact actions (sent/nudge/skip/let_go), annotations, skip-duration form, category fields, and research link
  5. **Yesterday's Reflection** — read-only
  6. **Today's Reflection** — editable, autosaved, with suggestions from completed tasks
- **Template layout source:** briefing `references/brief-artifact-template.html`. It uses stable `task-`, `outreach-`, and `event-` ids across HTML, Markdown, state, closures, and snooze ledgers.
...
- **Fallback audit (2026-09-18):** the portable input remains the local v0.7.0 state file, but briefing 0.12.0 does not yet specify the complete `suppressed`/`annotations` output, apply its derived `snoozed` array to the render filter (which reads the separate ledger), or route fallback `reflection.one_thing` to Center of Gravity (which reads the Markdown reflection). Cortex writes later findings to a `-remine` companion without a corresponding briefing reader. These are unresolved producer/consumer gaps, not a guarantee of full fallback parity; see the [candidate audit](../releases/2026.09.18-rc.1/NOTES.md).
...
- **Tomorrow seed contract:** cortex `/morning` Step 4.6 writes `<config-root>/briefs/<today>.seed.json` `{priorities:[...], outreach:[...]}`; optional `/end-day` may pre-stage tomorrow with the same shape. Briefing `/brief` reads it to seed Priority Tasks and Outreach Queue.
- **Data-flow trace:** artifact or chat actions → `.state.json` → `/listen` Step 1.5 mechanical ledgers + staged proposals → `/morning` review/merge → source nodes and future briefs. Annotation content in staged state may be sensitive; sanitize/paraphrase before committing it to durable active memory.
- **Version:** briefing v0.11.0 (state schema v0.7.0) + cortex v4.27.0 + growth v0.6.2, 2026-09-17.

### surfacing-prefs.md (cortex brief-mining writers ↔ briefing `/brief` reader)

`<config-root>/memory/me/surfacing-prefs.md`
- **Writers:** cortex `/listen` Step 1.5 stages not-important/repeat-ignore proposals for `/morning` review; optional `/end-day` Step 2c.3 applies the same semantics synchronously. Created from `cortex references/surfacing-prefs-template.md` if missing.
...
- **Related:** per-task skip counts in `<config-root>/memory/.brief-skip-counts.json` (`{task_id:{count,last_skipped,title}}`) are maintained mechanically by `/listen` Step 1.5h and optional `/end-day`.

### reflections.md (longitudinal reflection store)

`<config-root>/memory/me/reflections.md`
- **Writers:** cortex `/morning` merge path and optional `/end-day` Step 4.2 (append newest-first); artifact-authored reflections first round-trip through `/listen` and the brief twin. Created from `cortex references/reflections-template.md`.
